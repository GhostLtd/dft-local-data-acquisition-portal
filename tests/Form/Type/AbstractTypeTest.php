...
abstract class AbstractTypeTest extends TypeTestCase
{
    ...
    protected function debugShowFormStructure(FormInterface $form): void
    {
        $showFormStructure = function (FormInterface $form, int $indent) use (&$showFormStructure) {
            $indentStr = $indent === 0 ? '' : (str_repeat('  ', ($indent - 1)) . '\-');
            foreach($form as $child) {
                dump($indentStr . $child->getName());
                if ($child->count() > 0) {
                    $showFormStructure($child, $indent + 1);
                }
            }
        };

        $showFormStructure($form, 0);
    }

    protected function assertNoExtraData(FormInterface $form, string $path='root'): void {
        $this->assertEmpty($form->getExtraData(), "Unexpected extra data @ {$path}: ".json_encode($form->getExtraData()));
        foreach($form as $child) {
            $this->assertNoExtraData($child, "{$path}.{$child->getName()}");
        }
    }
}
